...
public class getNewJourneysClientStepDefinition {
...
public void new_journeys_are_found_and_located_in_the_new_journey_list() {
    assertFalse(client.getNewJourneys().isEmpty());
}

@Given("a client that has at least one journey1")
public void a_client_that_has_at_least_one_journey1() {
	client = new Client();
	journey = new Journey();
	journey = new Journey("origin", "destination", "content type", "company");
	client.addJourney(journey);
...
@When("the the user chooses to search for a new journey1")
public void the_the_user_chooses_to_search_for_a_new_journey1() {
	client.getNewJourneys();
}

@Then("the new journey list is empty and no new journeys are found")
public void the_new_journey_list_is_empty_and_no_new_journeys_are_found() {
